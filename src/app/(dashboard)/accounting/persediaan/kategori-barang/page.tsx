...
const dummyData: Kategori[] = [{ id: "1", nama: "General", kategoriDefault: true }]
type FormTab = "kategori" | "akun"

export default function KategoriBarangPage() {
  const [search, setSearch] = useState("")
  const [showForm, setShowForm] = useState(false)
  const [formTab, setFormTab] = useState<FormTab>("kategori")
  ...
  const tabs: { key: FormTab; label: string }[] = [
    { key: "kategori", label: "Kategori Barang" }, { key: "akun", label: "Akun" },
  ]

  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      <div style={{ padding: "12px 20px 0", background: "#fff" }}>
        <div><h1 style={{ fontSize: 20, fontWeight: 700, color: "#001526" }}>Kategori Barang</h1><p style={{ fontSize: 13, color: "#444746", marginTop: 2 }}>Kelola kategori barang</p></div>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 10, paddingBottom: 12 }}>
          <button onClick={() => { setShowForm(!showForm); setFormTab("kategori") }} style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 32, height: 32, background: "#0176d3", color: "#fff", border: "1px solid #0176d3", borderRadius: 6, cursor: "pointer" }}><Plus size={16} /></button>
            ...
            <div style={{ padding: "16px 20px", maxWidth: 500 }}>
              ...
              {formTab === "akun" && (
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  <label style={{ fontSize: 12, fontWeight: 600, color: "#0176d3" }}>Akun Perkiraan</label>
                  {[{l:"Persediaan"},{l:"Beban Pokok Penjualan"},{l:"Persediaan Terkirim"}].map(f => (
      ...
      <div style={{ flex: 1, overflow: "auto", background: "#fff" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
          <thead><tr style={{ background: "#f5f5f5" }}>
            ...
            <th style={{ padding: "8px 12px", textAlign: "left", fontWeight: 600, color: "#0176d3", borderBottom: "1px solid #e5e5e5", width: "50%" }}>Nama</th>
          ...
          <tbody>
            {filtered.map(item => (
              <tr key={item.id} style={{ borderBottom: "1px solid #f0f0f0", cursor: "pointer" }} onMouseEnter={(e) => e.currentTarget.style.background = "#f8f9ff"} onMouseLeave={(e) => e.currentTarget.style.background = "transparent"}>
                ...
                <td style={{ padding: "8px 12px", color: "#444746" }}>{item.kategoriDefault ? "Ya" : "Tidak"}</td>
              </tr>
            ))}
          </tbody>
        </table>
