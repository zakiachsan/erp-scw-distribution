...
export default function PenyelesaianPesananPage() {
  const [search, setSearch] = useState("")
  const [showForm, setShowForm] = useState(false)
  const [filterTipe, setFilterTipe] = useState("semua")
  const [formData, setFormData] = useState({
    tanggal: "07/07/2026", nomorOtomatis: true, tipeNomor: "Roll Over",
    jobNo: "", rollOverType: "Barang",
  })
  const filtered: any[] = []
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      <div style={{ padding: "12px 20px 0", background: "#fff" }}>
        <div><h1 style={{ fontSize: 20, fontWeight: 700, color: "#001526" }}>Penyelesaian Pesanan</h1><p style={{ fontSize: 13, color: "#444746", marginTop: 2 }}>Proses penyelesaian pesanan produksi</p></div>
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 10 }}>
          <select style={selectStyle}><option>Tanggal: Semua</option></select>
          <select value={filterTipe} onChange={(e) => setFilterTipe(e.target.value)} style={selectStyle}><option value="semua">Tipe Penyelesaian: Semua</option><option>Barang</option><option>Jasa</option></select>
        </div>
            ...
            <div style={{ borderTop: "1px solid #eee", paddingTop: 12 }}>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, marginBottom: 12 }}>
                <thead><tr style={{ background: "#4a5568" }}>
                  {["Item Name","Code#","Quantity","Unit","Portion (%)","Allocation Amount"].map(h => <th key={h} style={{ padding: "6px 10px", textAlign: h==="Quantity"||h==="Portion (%)"||h==="Allocation Amount"?"right":"left", fontWeight: 600, color: "#fff", borderBottom: "1px solid #3a4150" }}>{h}</th>)}
                </tr></thead>
      ...
      <div style={{ flex: 1, overflow: "auto", background: "#fff" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
          <thead><tr style={{ background: "#4a5568" }}>
            {[{l:"",w:"30px"},{l:"Nomor #",w:"13%"},{l:"Tanggal",w:"10%"},{l:"Pekerjaan Pesanan",w:"22%"},{l:"Tipe Penyelesaian",w:"16%"},{l:"Keterangan",w:"25%"}].map((c:any) => <th key={c.l||"x"} style={{ padding: "8px 10px", textAlign: "left", fontWeight: 600, color: "#fff", borderBottom: "1px solid #3a4150", width: c.w, fontSize: 11 }}>{c.l}</th>)}
          </tr></thead>
